...
@EnableWebSocket
public class WebSockConfig implements WebSocketConfigurer { //WebSocketMessageBrokerConfigurer : WebSocket 클라이언트로 받은 메세지 핸들링을 구성하는 메소드들이 정의할때 사용한다
//
...
////        registry.addEndpoint("/ws").setAllowedOrigins("*").withSockJS(); // "/ws" 는 WebSocket 클라이언트가 Handshake를 위해 연결해야하는 end-point의 url
//
//    }

    @Override
    public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
        registry.addHandler(webSocketHandler,"/ws/chat").setAllowedOrigins("*"); // "/ws" 는 WebSocket 클라이언트가 Handshake를 위해 연결해야하는 end-point의 url
    }
}
